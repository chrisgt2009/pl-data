...
# Single-season mode (backwards compatible)
YEAR = os.getenv("F1_SEASON", "2013")
OUT_DIR = Path(os.getenv("F1_OUT_DIR", f"data/f1/{YEAR}"))

# Range mode (new, optional)
SEASON_FROM = os.getenv("F1_SEASON_FROM")
SEASON_TO = os.getenv("F1_SEASON_TO")
OUT_ROOT = Path(os.getenv("F1_OUT_ROOT", "data/f1"))
...
def fetch_drivers_by_ids(driver_ids: list[int], season_label: str, year_dir: Path) -> dict:
    """
    ...
    out = {
        "get": "drivers",
        "parameters": {"ids": driver_ids, "season_source": season_label},
        "errors": {},
        "results": 0,
        "response": [],
    }
...
def process_one_season(season: str, out_dir: Path) -> None:
    ensure_dir(out_dir)

    jobs = [
        ("seasons", {}, out_dir / "seasons.json"),

        ("races", {"season": season}, out_dir / "races.json"),
        ("rankings/drivers", {"season": season}, out_dir / "standings_drivers.json"),
        ("rankings/teams", {"season": season}, out_dir / "standings_teams.json"),

        ("circuits", {"season": season}, out_dir / "circuits.json"),
        ("teams", {"season": season}, out_dir / "teams.json"),
    ]
    ...
    if not driver_ids:
        ...
        drivers_payload = fetch_drivers_by_ids(driver_ids, season, out_dir)
...
def main() -> None:
    # ✅ Range mode (if both vars exist)
    if SEASON_FROM and SEASON_TO:
        try:
            y1 = int(SEASON_FROM)
            y2 = int(SEASON_TO)
        except ValueError:
            print("ERROR: F1_SEASON_FROM and F1_SEASON_TO must be integers", file=sys.stderr)
            sys.exit(1)

        if y1 > y2:
            print("ERROR: F1_SEASON_FROM must be <= F1_SEASON_TO", file=sys.stderr)
            sys.exit(1)
        ...
        for y in range(y1, y2 + 1):
            ...
            print(f"\n🏁 Processing season {season} -> {out_dir}")
            process_one_season(season, out_dir)

        return

    # ✅ Backwards compatible single-season mode
    process_one_season(YEAR, OUT_DIR)
